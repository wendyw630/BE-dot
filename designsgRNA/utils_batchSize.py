from BE_dot.dot_baseEditorsTable import CBElist,  ABElist, BEletter
from BE_dot.snpClass import *
...
def syn3_correct(snp):
    ...
    for i in [0,1]:
        ...
        if supo_snp.wt=='C':
            ...
            BElist=None
        if i==0:### !!!!!!!!!!!!!!!!!!!! to give an initila-value & inherit i==0 when i==1
            for BE in BElist:
...
def Main(snpfile):
    ...
    snps = importSNPS(snpfile)
    #@  print('snps in snpfilein:',snps)
    """
    snps=[]
    newsnp=snp_define('rs28937596', 'CTTTCTTCCATAGCTGCTAAAGGCC', 'GGAGCCCTGTTTTTAGGAACTACAT', 'T','C', 1)
    snps.append(newsnp)
    """
    ...
    #@ print("********* allmatch:\n", allmatch)
    ################  writeout  ##################
    with open(r'D:\00_project\predict\BE_dot\results\cleanMatches_file.txt', mode='w') as fout:
    ...
    with open(r'D:\00_project\predict\BE_dot\results\quietMatches_file.txt', mode='w') as fout:
    ...
    with open(snpfile,mode='r') as file:
        file_read = csv.reader(file, delimiter=",")
        ##########   use {total_table} to note first annote-cols of each row  #############
        total_table={}
        for row in file_read:
            ...
            ###############################################
    with open(r'D:\00_project\predict\BE_dot\results\full_results.csv', mode='w',newline='') as f:
        ...
        #############   use [mlist] to note BE-usage of each row  ###############
        canmatch_snp=allmatch.keys() ###.......allmatch.........
        ...
        for snp in canmatch_snp:
            matchTag_l=[]
            for be in beList:
                tmp=''
                if snp in synonymous123:
                    if be in synonymous123[snp]:
                        if synonymous123[snp][be]!=[]:
                            tmp="Synonymous"
                if snp in precise:
                    if be in precise[snp]:
                        if precise[snp][be]!=[]:
                            tmp = "Precise"
                matchTag_l.append(tmp)
            ######################  final write-rows  ####################
            ##########   add first annote-cols of each row  #############
            #@  print("##########matchTag_l:",matchTag_l,"\nlen(matchTag_l):",len(matchTag_l))
            if matchTag_l!=['']*38: ### wzl_add 4 BEs, in FF *34
                a=[snp, total_table[snp]['genome_assembly'], total_table[snp]['chromosome'],
                total_table[snp]['orientation'], total_table[snp]['start_position'],
                total_table[snp]['end_position'], total_table[snp]['xmer'], total_table[snp]['clinical_sig'], total_table[snp]['condition'],
                total_table[snp]['snp'],
                total_table[snp]['residue'], total_table[snp]['var_type'],
                total_table[snp]['gene_name'], total_table[snp]['gene_ID'],
                total_table[snp]['reading_frame'], total_table[snp]['aaPosition']]
                for m in matchTag_l:
                    a.append(m)
                f.writerow(a)

if __name__=='__main__':
    Main(r'D:\00_project\predict\BEFF\snps_with_clinvar2.csv')